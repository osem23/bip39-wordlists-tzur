...
- **Native-speaker review is incomplete.** Hebrew has full native-speaker review. The other 29 TZUR Original wordlists were built through translation, structural validation, and a translation-accuracy audit. The audit combines mechanical validation, back-translation via Google Translate, and, for 19 of them, forward-translation via Microsoft Azure Translator, with each translation pass paired with an LLM verdict layer. See [`docs/CONSTRUCTION.md`](docs/CONSTRUCTION.md) for per-language audit depth and refinement counts. Native-speaker review is the gold standard for idiom, register, and cultural neutrality; contributions land via PR.
...
- **No external cryptographic or linguistic audit.** Validation in this repository is automated (format, encoding, round-trip integrity) and includes the translation-accuracy audit described in `docs/CONSTRUCTION.md`. Independent third-party review has not been performed.
