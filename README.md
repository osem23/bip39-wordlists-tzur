...
## Manual recovery (display backup to English, by hand)

A display backup converts back to the canonical English mnemonic by index, with no tooling beyond the wordlist files. This is the manual recovery path a user (or a support engineer) can follow if they have only a display-language phrase and want to restore in a standard English-only BIP-39 wallet:

1. Take the display phrase, word by word, in order.
2. For each display word, find its line number in that language's wordlist at [`wordlists/tzur-original/<language>.txt`](wordlists/tzur-original/). That line number is the word's index. (Equivalently, look the word up in the mapping's `native_to_english` and skip to step 4.)
3. Read the English word at the **same** index in [`wordlists/reference-canonical/english.txt`](wordlists/reference-canonical/english.txt).
4. The English words, in the same order, are the canonical English BIP-39 mnemonic.
5. Enter that English mnemonic into any standard BIP-39 wallet to recover the funds.

Because the indices are identical on both sides, the recovered English phrase carries the same BIP-39 checksum as the display phrase and is accepted by any BIP-39 wallet without modification. Recovery still depends on the wallet using the **same derivation path** the wallet was created under (TZUR uses BIP-84, `m/84'/0'/0'`) and the **same BIP-39 passphrase**, if one was set. The passphrase is never translated or mapped by this convention; it is entered exactly as the user originally typed it.

The bidirectional [`mappings/`](mappings/) files make this a one-step lookup in either direction; the line-number method above is the fallback that needs only the two plain-text wordlists.
...
- Normalization: NFC at rest, NFKD before PBKDF2 as required by BIP-39. NFC and NFKD never merge two distinct entries (zero NFKD collisions across all 30 lists); wallets that additionally strip diacritics or case-fold input must reject the resulting ambiguity rather than guess. See [`validation/encoding-notes.md`](validation/encoding-notes.md).
- 4-char prefix uniqueness is not guaranteed across all scripts; it holds only for Korean. Wallets relying on prefix autocomplete should fall back to full-word matching. Per-language figures in [`docs/prefix-statistics.md`](docs/prefix-statistics.md).
...
- [`docs/prefix-statistics.md`](docs/prefix-statistics.md). Per-language 2/3/4-character prefix uniqueness and largest collision groups, quantifying where prefix autocomplete is safe and where wallets must fall back to full-word matching. Generated by [`validation/prefix_stats.py`](validation/prefix_stats.py).
...
**Is the BIP-39 checksum still valid for a native-language phrase?**
Yes. The checksum is carried by the word indices, and this convention preserves indices exactly: the native word at index N maps back to the English word at index N, so a display phrase resolves to the same index sequence, the same English mnemonic, and the same checksum as the phrase it renders. The display layer never recomputes or relaxes the checksum. A single transcription error in a display phrase fails the checksum exactly as it would in English.

**How do I convert a native-language backup back to English by hand?**
Look up each display word's line number in its wordlist, read the English word at the same line number in [`wordlists/reference-canonical/english.txt`](wordlists/reference-canonical/english.txt), and the English words in order are your canonical mnemonic. Recovery still needs the same derivation path and BIP-39 passphrase. See [Manual recovery](#manual-recovery-display-backup-to-english-by-hand).
