...
- **Governance.** `docs/GOVERNANCE.md` defines the repository's versioning model (single force-moveable `v1.0` tag, SHA-256 as load-bearing identifier), what changes the SHA-256, what constitutes a breaking change for integrators, the communication channel for changes, the process for wordlist updates, and the migration path if the registry ever needs immutable versioned tags.
